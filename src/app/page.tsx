...
import { Shield, Activity, Brain, AlertTriangle, ArrowRight, Zap, Globe, TrendingUp, Target, Play, Star, Sparkles, Rocket, Users, Building2, CheckCircle2, ArrowDown } from 'lucide-react'
    ...
    <div className="min-h-screen bg-background relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-6">
              <Link href="/dashboard.html" className="text-sm hover:text-primary">Dashboard</Link>
              <Link href="/command-center.html" className="text-sm hover:text-primary">Command Center</Link>
              <Link href="/subscription.html" className="text-sm hover:text-primary">Pricing</Link>
          ...
          <div className="grid lg:grid-cols-[1fr_auto] gap-8 items-center">
            ...
            <div className="max-w-2xl text-center lg:text-left">
              ...
              <p className="text-lg md:text-xl text-muted-foreground mb-10 max-w-xl mx-auto lg:mx-0 leading-relaxed">
                Enterprise-grade AI platform with <span className="text-primary font-semibold">94% accuracy</span> across <span className="text-cyan-400 font-semibold">190+ countries</span>.
              </p>
              ...
              <div className="flex flex-col sm:flex-row items-center lg:items-start gap-4 mb-10">
                ...
                <Link href="/command-center.html">
                  <Button variant="outline" size="lg" className="gap-2 px-10 h-14 border-2 hover:bg-muted group text-base w-full sm:w-auto">
                    <Activity className="h-5 w-5" /> Command Center
                  </Button>
                </Link>
              </div>
      ...
      <section className="py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <Badge variant="secondary" className="mb-4 gap-2"><Zap className="h-4 w-4 text-primary" /> Enterprise AI Suite</Badge>
            <h2 className="text-4xl md:text-5xl font-bold mb-4">Powerful Tools at Your Fingertips</h2>
            <p className="text-xl text-muted-foreground max-w-3xl mx-auto">Interactive features with real-time data integration</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <Card className="group hover:border-primary/50 transition-all hover:shadow-lg">
              <CardContent className="p-6">
                <div className="p-3 bg-gradient-to-br from-violet-500 to-purple-600 rounded-xl w-fit mb-4">
                  <Brain className="h-6 w-6 text-white" />
                </div>
                <h3 className="text-xl font-bold mb-2">AI Risk Prediction</h3>
                <p className="text-muted-foreground mb-4">Weather-based logistics disruption analysis with OpenWeatherMap integration</p>
                <Badge className="bg-emerald-500/10 text-emerald-400">LIVE DATA</Badge>
              </CardContent>
            </Card>

            {/* Global Supply Mapping */}
            <Card className="group hover:border-primary/50 transition-all hover:shadow-lg">
              <CardContent className="p-6">
                <div className="p-3 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-xl w-fit mb-4">
                  <Globe className="h-6 w-6 text-white" />
                </div>
                <h3 className="text-xl font-bold mb-2">Global Supply Mapping</h3>
                <p className="text-muted-foreground mb-4">Interactive supplier network visualization with 190+ countries</p>
                <Badge className="bg-blue-500/10 text-blue-400">INTERACTIVE</Badge>
              </CardContent>
            </Card>
            ...
            <Card className="group hover:border-primary/50 transition-all hover:shadow-lg">
              <CardContent className="p-6">
                <div className="p-3 bg-gradient-to-br from-emerald-500 to-green-600 rounded-xl w-fit mb-4">
                  <Shield className="h-6 w-6 text-white" />
                </div>
                <h3 className="text-xl font-bold mb-2">Compliance Automation</h3>
                <p className="text-muted-foreground mb-4">Real-time regulation checker for GDPR, SOC 2, ISO 27001</p>
                <Badge className="bg-violet-500/10 text-violet-400">AUTOMATED</Badge>
              </CardContent>
            </Card>
          </div>
        </div>
      ...
      <section className="py-24 bg-muted/30" id="pricing">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <Badge variant="secondary" className="mb-4 gap-2"><Star className="h-4 w-4 text-primary" /> Flexible Pricing</Badge>
            <h2 className="text-3xl md:text-4xl font-bold mb-4">Choose Your Plan</h2>
          </div>

          <div className="grid md:grid-cols-3 gap-8">
            {[
              { name: 'Starter', price: '$0', features: ['100 predictions/mo', 'Basic analytics', 'Email support'], cta: 'Get Started Free' },
              { name: 'Professional', price: '$99', popular: true, features: ['1,000 predictions/mo', 'Advanced analytics', 'Priority support', 'API access'], cta: 'Start Trial' },
              { name: 'Enterprise', price: 'Custom', features: ['Unlimited predictions', 'Dedicated support', 'Custom integrations', 'SLA guarantee'], cta: 'Contact Sales' },
            ].map((plan, i) => (
              <Card key={i} className={`relative ${plan.popular ? 'border-primary shadow-lg' : ''}`}>
                {plan.popular && <div className="absolute -top-3 left-1/2 -translate-x-1/2"><Badge>Most Popular</Badge></div>}
                <CardContent className="p-6">
                  <h3 className="text-xl font-bold">{plan.name}</h3>
                  <div className="text-4xl font-bold my-4">{plan.price}<span className="text-base text-muted-foreground">/mo</span></div>
                  <ul className="space-y-2 mb-6">
                    {plan.features.map((f, j) => (<li key={j} className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-emerald-400" /><span className="text-sm">{f}</span></li>))}
                  </ul>
                  <Button className={`w-full ${plan.popular ? '' : 'variant-outline'}`}>{plan.cta}</Button>
                </CardContent>
              </Card>
            ))}
          </div>
      ...
      <section className="py-24">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-lg text-muted-foreground mb-8">Join 500+ enterprises using AI to predict disruptions</p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link href="/dashboard.html"><Button size="lg" className="gap-2 px-10">Start Free Trial <ArrowRight className="h-5 w-5" /></Button></Link>
            <Link href="/subscription.html"><Button size="lg" variant="outline" className="gap-2 px-10">View Pricing</Button></Link>
          </div>
        </div>
      </section>
      ...
      <footer className="border-t border-border py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-4 gap-8 mb-8">
            <div><div className="flex items-center gap-2 mb-4"><Shield className="h-6 w-6 text-primary" /><span className="font-bold">AI Supply Chain</span></div><p className="text-sm text-muted-foreground">Enterprise-grade AI supply chain risk prediction.</p></div>
            <div><h4 className="font-semibold mb-4">Product</h4><ul className="space-y-2 text-sm text-muted-foreground"><li><Link href="/dashboard.html">Dashboard</Link></li><li><Link href="/command-center.html">Command Center</Link></li><li><Link href="/subscription.html">Pricing</Link></li></ul></div>
            <div><h4 className="font-semibold mb-4">Company</h4><ul className="space-y-2 text-sm text-muted-foreground"><li><Link href="/about.html">About</Link></li><li><Link href="/customers.html">Customers</Link></li><li><Link href="/support.html">Support</Link></li></ul></div>
            <div><h4 className="font-semibold mb-4">Legal</h4><ul className="space-y-2 text-sm text-muted-foreground"><li><a href="#">Privacy Policy</a></li><li><a href="#">Terms of Service</a></li><li><a href="#">GDPR</a></li></ul></div>
          </div>
          <div className="border-t border-border pt-8 text-center text-sm text-muted-foreground">
            © 2024 AI Supply Chain Risk Predictor. All rights reserved.
          </div>
        </div>
      </footer>
    </div>
